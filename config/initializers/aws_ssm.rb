...
def fetch_params_list(ssm_client)
    ...
    begin
      loop do
        ...
        params_list += response.parameters.map(&:name).select { |name| name.include?("/conclave-document-upload/") }
...
def set_env(ssm_client, params_list)
    params_list.each do |param_name|
        ...
        env_param_name = param_name.gsub("/conclave-document-upload/", "").to_s
        ENV[env_param_name.upcase] = response.parameter.value
        puts " 1 #{ENV[env_param_name.upcase]}"
    end
end 
