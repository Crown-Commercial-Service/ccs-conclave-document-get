class ApplicationController < ActionController::API
  ...
  def validate_client_id
    ...
    raise ActionController::BadRequest, 'Not authorized: Missing Client ID' if decoded_token[0]['aud'].blank?
  end

  def validate_user_access_token
    raise ActionController::BadRequest, 'Not authorized: Missing Access Token' if bearer_token(request.headers).blank?
  end
